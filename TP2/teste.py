...
from PyQt5.QtWidgets import QApplication, QMainWindow, QGraphicsScene, QGraphicsView, QGraphicsLineItem, QGraphicsPolygonItem, QGraphicsEllipseItem, QPushButton, QVBoxLayout, QWidget, QHBoxLayout  # type: ignore
...
class MyWindow(QMainWindow):
    ...
    def add_point(self):
        # Add a new point at the center of the window
        new_point = Ponto2D((self.window.xwmin + self.window.xwmax) / 2, (self.window.ywmin + self.window.ywmax) / 2)
        self.objetos_transformados.append(new_point)
        self.transform_objects()
        self.display_objects()

    def add_line(self):
        # Add a new line at the center of the window
        center_x = (self.window.xwmin + self.window.xwmax) / 2
        center_y = (self.window.ywmin + self.window.ywmax) / 2
        new_line = Reta2D(Ponto2D(center_x - 1, center_y - 1), Ponto2D(center_x + 1, center_y + 1))
        self.objetos_transformados.append(new_line)
        self.transform_objects()
        self.display_objects()

    def add_polygon(self):
        # Add a new triangle at the center of the window
        center_x = (self.window.xwmin + self.window.xwmax) / 2
        center_y = (self.window.ywmin + self.window.ywmax) / 2
        new_polygon = Poligono2D([
            Ponto2D(center_x, center_y + 1),
            Ponto2D(center_x - 1, center_y - 1),
            Ponto2D(center_x + 1, center_y - 1)
        ])
